...
{   
    ...
    this->sys_clock = (float)clock_get_hz(clk_sys);
    // Rejestracja instancji w odpowiedniej tablicy PIO 
    if(PIO_instance == pio0)
    {
        for(int i = 0; i < LENGHT; i++)
        {
...
void Stepper::loadToPIO(long steps, float speed) {
    ...
    while (pio_sm_is_tx_fifo_full(PIO_instance, SM_speed)) 
    {
        if (!this->Enable) return; 
    }
    pio_sm_put(PIO_instance, SM_speed, delay_cycles);

    while (pio_sm_is_tx_fifo_full(PIO_instance, SM_counter)) 
    {
        if (!this->Enable) return; 
    }
    pio_sm_put(PIO_instance, SM_counter, (uint32_t)abs(steps));
       ...
       if(remaining_steps <= 0)
        {
            ...
            remaining_steps = abs(next_steps);
            steps_direction = (next_steps >= 0) ? 1 : -1;

            if(ext_sync)
            {
                maxSpeed = true;
                actual_speed_multiplier = ext_mult;
            }
            else
            {
                if(!isMoving) 
                ...
                {
                    maxSpeed = true;
                    actual_speed_multiplier = 1.0f;
                }
            }

        }

        long steps_to_send = 0;
        float base_div = sys_clock / 2000000.0f;
        long ramp_chunk = 50;

        float final_multiplier = actual_speed_multiplier * Stepper::global_override;

        if(!maxSpeed)
        {
            steps_to_send = (remaining_steps > ramp_chunk) ? ramp_chunk : remaining_steps;
            pio_sm_set_clkdiv(PIO_instance, SM_speed, base_div / final_multiplier);
...
void Stepper::set_global_override(float val) {
    if(val < 0.05f) val = 0.05f; 
    if(val > 2.0f) val = 2.0f;   
    global_override = val;
    float base_div = (float)clock_get_hz(clk_sys) / 2000000.0f;

    for(int i = 0; i < LENGHT; i++) {
        if(PIO0_Instance[i] != nullptr && PIO0_Instance[i]->isMoving) {
            float final_mult = PIO0_Instance[i]->actual_speed_multiplier * global_override;
            pio_sm_set_clkdiv(pio0, PIO0_Instance[i]->SM_speed, base_div / final_mult);
        }
    }

    for(int i = 0; i < LENGHT; i++) {
        if(PIO1_Instance[i] != nullptr && PIO1_Instance[i]->isMoving) {
            float final_mult = PIO1_Instance[i]->actual_speed_multiplier * global_override;
            pio_sm_set_clkdiv(pio1, PIO1_Instance[i]->SM_speed, base_div / final_mult);
        }
    }

    for(int i = 0; i < LENGHT; i++) {
        if(PIO2_Instance[i] != nullptr && PIO2_Instance[i]->isMoving) {
            float final_mult = PIO2_Instance[i]->actual_speed_multiplier * global_override;
            pio_sm_set_clkdiv(pio2, PIO2_Instance[i]->SM_speed, base_div / final_mult);
        }
    }
}
