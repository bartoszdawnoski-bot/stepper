...
void GCode::execute_parse()
{
    ...
    if(parser.HasWord('G'))
    {
        ...
        if(num == 1 || num == 0)
        {
            ...
            bool is_stopped = ( !stepperX->moving() && stepperX->isBufferEmpty() ) && ( !stepperY->moving() && stepperY->isBufferEmpty() );
            if(stepperZ != nullptr) is_stopped = is_stopped && ( !stepperZ->moving() && stepperZ->isBufferEmpty() );

            if(is_stopped) accel_dist = 0;;
            int segments = (int)ceil(total_dist / MAX_SEGMENT_MM);
            if(segments < 1) segments = 1;

            float steps_x = diffX / segments;
            float steps_y = diffY / segments;
            float steps_z = (stepperZ != nullptr) ? diffZ / segments : 0;
            
            long acc_x = 0, acc_y = 0, acc_z = 0;
            float total_dist_seg = total_dist / segments;
            
            for (size_t i = 1; i < segments; i++)
            {
                float p = accel_dist / ACCEL_RAMP_MM;
                if(p > 1) p = 1;

                float s_curve = (p * p) * (3.0f - 2.0f * p); 
                float accel_factor = START_SPEED_FACTOR + (1.0f - START_SPEED_FACTOR) * s_curve;

                long current_x = (long)(steps_x * i) - acc_x;
                long current_y = (long)(steps_y * i) - acc_y;
                long current_z = (stepperZ != nullptr) ? (long)(steps_z * i) - acc_z : 0;

                acc_x += current_x;
                acc_y += current_y;
                acc_z += current_z;

                float current_vX = vX * accel_factor;
                float current_vY = vY * accel_factor;
                float current_vZ = vZ * accel_factor;

                while((current_x != 0 && stepperX->isBufferFull()) || (current_y != 0 && stepperY->isBufferFull()) || (stepperZ != nullptr && current_z != 0 && stepperZ->isBufferFull()))
                {
                    move_complete();
                }
                if (current_x != 0) stepperX->addMove(current_x, current_vX);
                if (current_y != 0) stepperY->addMove(current_y, current_vY);
                if (stepperZ != nullptr && current_z != 0) stepperZ->addMove(current_z, current_vZ);
            }
