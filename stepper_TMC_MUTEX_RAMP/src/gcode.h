...
private:
    ...
    //Przyśpieszenie
    const float MAX_SEGMENT_MM = 1.0f; // Maksymalny segment
    const float ACCEL_RAMP_MM = 3.0f; // Maszyna osiągnie 100% prędkości po dokładnie 3 mm
    const float START_SPEED_FACTOR = 0.15f; // Prędkość startowa (15%)
    float accel_dist = 0.0f; // Przebyty dystans od momentu startu
